...
<?php
	...
	function download_orderdetail_list() {
		...
		$chk_pro['tax'] = ( isset( $_REQUEST['check']['tax'] ) && usces_is_tax_display() ) ? 1 : 0;
		...
		if( isset( $_REQUEST['check']['tax'] ) && usces_is_tax_display() && 'products' == usces_get_tax_target() ) $line .= $th_h.usces_tax_label( array(), 'return' ).$th_f;
		...
		if( isset( $_REQUEST['check']['tax'] ) && usces_is_tax_display() && 'all' == usces_get_tax_target() ) $line .= $th_h.usces_tax_label( array(), 'return' ).$th_f;
		...
		foreach( (array)$rows as $data ) {
			...
			for( $i = 0; $i < $cart_count; $i++) {
				...
				if( isset( $_REQUEST['check']['tax'] ) && usces_is_tax_display() && 'products' == usces_get_tax_target() ) {
				...
				if( isset( $_REQUEST['check']['tax'] ) && usces_is_tax_display() && 'all' == usces_get_tax_target() ) {
					if( 'include' == usces_get_tax_mode() ) {
						$materials = array(
							'total_items_price' => $data['item_total_price'],
							'discount' => $data['discount'],
	...
	function download_order_list() {
		...
		$chk_ord['tax'] = ( isset( $_REQUEST['check']['tax'] ) && usces_is_tax_display() ) ? 1 : 0;
