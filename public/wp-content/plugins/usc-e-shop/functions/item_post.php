			...
			<tr>
				<td colspan='5' class='submittd'>
					<div id='skusubmit-<?php echo $id; ?>' class='submit'>
						...
						<?php usces_sku_meta_row_reduced_tax_rate( $sku ); ?>
					</div>
		...
		<tr>
			<td colspan="5" class="submittd">
				<div id='newskusubmit' class='submit'>
					...
					?>
					<?php usces_newsku_meta_row_reduced_tax_rate(); ?>
				</div>
...
function order_item_ajax(){
	...
	switch ( $_POST['mode'] ) {
		...
		case 'getmember':
			...
			break;
		case 'recalculation':
			$res = usces_order_recalculation( $_POST['order_id'], $_POST['mem_id'], $_POST['post_ids'], $_POST['skus'], $_POST['prices'], $_POST['quants'], $_POST['upoint'], $_POST['shipping_charge'], $_POST['cod_fee'], $_POST['discount'] );
			break;
		case 'recalculation_reduced':
			$res = usces_order_recalculation_reduced( $_POST['order_id'], $_POST['mem_id'], $_POST['post_ids'], $_POST['prices'], $_POST['quants'], $_POST['cart_ids'], $_POST['upoint'], $_POST['shipping_charge'], $_POST['cod_fee'], $_POST['discount'] );
			break;
...
function usces_order_recalculation( $order_id, $mem_id, $post_id, $skus, $price, $quant, $use_point, $shipping_charge, $cod_fee, $discount = 0 ) {
	global $usces;
	...
	$total_price = apply_filters( 'usces_filter_order_total_price_recalculation', $total_price, $total_items_price, $use_point, $discount, $shipping_charge, $cod_fee, $cart );
	...
	$total_full_price = apply_filters( 'usces_filter_order_total_full_price_recalculation', $total_full_price, $total_items_price, $use_point, $discount, $shipping_charge, $cod_fee, $cart );
...
function usces_order_recalculation_reduced( $order_id, $mem_id, $post_id, $price, $quant, $cart_id, $use_point, $shipping_charge, $cod_fee, $discount = 0 ) {
	...
	for( $i = 0; $i < $post_id_count; $i++ ) {
		if( $post_id[$i] ) {
			...
			$ordercartmeta = usces_get_ordercart_meta( 'tax', $cart_id[$i] );
			if( $ordercartmeta ) {
				$tax = $ordercartmeta[0]['meta_key'];
				$tax_rate = $ordercartmeta[0]['meta_value'];
			} else {
				$tax = 'standard';
	...
	if( 'include' == $usces->options['tax_mode'] ) {
		$tax_standard = (float)sprintf( '%.3f', (float)$subtotal_standard * (float)$usces->options['tax_rate'] / ( 100 + (float)$usces->options['tax_rate'] ) );
		$tax_reduced = (float)sprintf( '%.3f', (float)$subtotal_reduced * (float)$usces->options['tax_rate'] / ( 100 + (float)$usces->options['tax_rate'] ) );
	} else {
		$tax_standard = (float)sprintf( '%.3f', $subtotal_standard * (float)$usces->options['tax_rate'] / 100 );
		$tax_reduced = (float)sprintf( '%.3f', $subtotal_reduced * (float)$usces->options['tax_rate_reduced'] / 100 );
	}
	...
	switch( $usces->options['tax_method'] ) {
		case 'cutting':
			$tax_standard = floor( $tax_standard * $decipad ) / $decipad;
			$tax_reduced = floor( $tax_reduced * $decipad ) / $decipad;
			break;
		case 'bring':
			$tax_standard = ceil( $tax_standard * $decipad ) / $decipad;
			$tax_reduced = ceil( $tax_reduced * $decipad ) / $decipad;
			break;
		case 'rounding':
			if( 0 < $decimal ) {
				$tax_standard = round( $tax_standard, $decimal );
				$tax_reduced = round( $tax_reduced, $decimal );
			} else {
				$tax_standard = round( $tax_standard );
				$tax_reduced = round( $tax_reduced );
			}
			break;
	}
	...
	$data['point'] = $point;
	$data['total_full_price'] = usces_crform( $total_full_price, false, false, 'return', false );
	$data['subtotal_standard'] = usces_crform( $subtotal_standard, false, false, 'return', false );
	$data['subtotal_reduced'] = usces_crform( $subtotal_reduced, false, false, 'return', false );
	$data['tax_standard'] = usces_crform( $tax_standard, false, false, 'return', false );
	$data['tax_reduced'] = usces_crform( $tax_reduced, false, false, 'return', false );
	wp_send_json( $data );
}

function usces_sku_meta_row_reduced_tax_rate( $sku ) {
	global $usces;
	...
	?>
		<select id="itemsku[<?php echo $id; ?>][applicable_tax_rate]" name="itemsku[<?php echo $id; ?>][applicable_tax_rate]" class="sku_applicable_tax_rate" >
...
function usces_newsku_meta_row_reduced_tax_rate() {
	global $usces;
	...
	if( $usces->is_reduced_tax_rate() ) :
		...
		$reduced = $usces->options['tax_rate_reduced'];
	?>
		<select id="newsku_applicable_tax_rate" name="newsku_applicable_tax_rate" class="newsku_applicable_tax_rate" >
