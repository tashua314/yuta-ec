...
function usces_order_confirm_message($order_id) {
	...
	$tax_rates_mixed = ( usces_is_tax_display() ) ? $usces_tax->is_tax_rates_mixed( $cart ) : false;
	...
	foreach ( (array)$cart as $cart_row ) {
		...
		if( $tax_rates_mixed ) {
			$applicable_tax_rate = $usces_tax->get_ordercart_applicable_tax_rate( $cart_row['cart_id'] );
			if( 'reduced' == $applicable_tax_rate ) {
				$cartItemName .= $usces_tax->reduced_tax_rate_mark;
			}
		}
	...
	$meisai .= usces_mail_line( 2, $data['order_email'] );//--------------------
	if( $tax_rates_mixed ) {
		$usces_tax->get_tax_order_cart( $cart );
		$meisai .= sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate'] ) . " : " . usces_crform( $usces_tax->subtotal_standard, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate'] ) . " : " . usces_crform( $usces_tax->tax_standard, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate_reduced'] ) . " : " . usces_crform( $usces_tax->subtotal_reduced, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate_reduced'] ) . " : " . usces_crform( $usces_tax->tax_reduced, true, false, 'return' ) . "\r\n";
		$meisai .= usces_mail_line( 2, $data['order_email'] );//--------------------
		$meisai .= $usces_tax->reduced_tax_rate_mark . __( ' is reduced tax rate', 'usces' ) . "\r\n";
	}
...
function usces_send_ordermail($order_id) {
	...
	foreach ( $cart as $cart_key => $cart_row ) {
		...
		if( $tax_rates_mixed ) {
			$applicable_tax_rate = $usces_tax->get_sku_applicable_tax_rate( $post_id, $sku );
			if( 'reduced' == $applicable_tax_rate ) {
				$cartItemName .= $usces_tax->reduced_tax_rate_mark;
			}
		}
	...
	$meisai .= usces_mail_line( 2, $entry['customer']['mailaddress1'] );//--------------------
	if( $tax_rates_mixed ) {
		$usces_tax->get_tax_entry_cart( $cart );
		$meisai .= sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate'] ) . " : " . usces_crform( $usces_tax->subtotal_standard, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate'] ) . " : " . usces_crform( $usces_tax->tax_standard, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate_reduced'] ) . " : " . usces_crform( $usces_tax->subtotal_reduced, true, false, 'return' ) . "\r\n";
		$meisai .= sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate_reduced'] ) . " : " . usces_crform( $usces_tax->tax_reduced, true, false, 'return' ) . "\r\n";
...
function usces_reg_orderdata( $results = array() ) {
	...
	if( usces_is_tax_display() && $usces->is_reduced_tax_rate() ) {
		$usces_tax = Welcart_Tax::get_instance();
		if( $usces_tax->is_tax_rates_mixed( $cart ) ) {
			$usces_tax->get_tax_entry_cart( $cart );
			...
			$usces->set_order_meta_value( 'subtotal_standard', $item_total_price, $order_id );
			if( 'include' == $usces->options['tax_mode'] ) {
				$usces->set_order_meta_value( 'tax_standard', 0, $order_id );
			} else {
				$usces->set_order_meta_value( 'tax_standard', $entry['order']['tax'], $order_id );
			}
		}
	}
	...
	if( usces_is_tax_display() && $usces->is_reduced_tax_rate() ) {
		...
		if( $usces_tax->is_tax_rates_mixed( $cart ) ) {
			...
			if( 'include' == $usces->options['tax_mode'] ) {
				...
				$usces->set_order_meta_value( 'tax_standard', $_POST['offer']['tax'], $order_id );
			}
		}
	}
...
function usces_update_orderdata() {
	...
	if( usces_is_tax_display() && $usces->is_reduced_tax_rate() ) {
		...
		$usces_tax = Welcart_Tax::get_instance();
		if( $usces_tax->is_tax_rates_mixed( $new_cart ) ) {
			$usces_tax->get_tax_order_cart( $new_cart );
			...
			$usces->set_order_meta_value( 'subtotal_standard', $item_total_price, $ID );
			if( 'include' == $usces->options['tax_mode'] ) {
				$usces->set_order_meta_value( 'tax_standard', 0, $ID );
			} else {
				$usces->set_order_meta_value( 'tax_standard', $_POST['offer']['tax'], $ID );
			}
		}
	}
...
function usces_get_ordercart_row( $order_id, $cart = array() ){
	...
	foreach( $cart as $i => $cart_row ) { 
		...
		$applicable_tax_rate = $usces_tax->get_ordercart_applicable_tax_rate( $ordercart_id );
		if( 'reduced' == $applicable_tax_rate ) {
			$reduced_tax_rate_mark = '<span class="reduced_tax_rate_mark">'.$usces_tax->reduced_tax_rate_mark.'</span>';
		} else {
			$reduced_tax_rate_mark = '';
		}
...
?>
	<tr>
		...
		<td class="aleft"><?php echo apply_filters('usces_filter_admin_cart_item_name', esc_html($cartItemName), $materials ); ?><?php echo $reduced_tax_rate_mark; ?><?php do_action('usces_admin_order_item_name', $order_id, $i); ?><?php usces_make_option_field( $materials, $cart ); ?></td>
...
<?php if( $usces->is_reduced_tax_rate() ): ?>
		<input name="applicable_tax_rate[<?php echo $ordercart_id; ?>]" type="hidden" value="<?php echo esc_attr( $applicable_tax_rate ); ?>" />
<?php endif; ?>
