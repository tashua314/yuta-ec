...
class usc_e_shop
{
	...
	function __construct()
	{
		...
		if(!isset($this->options['applicable_tax_rate']) || empty($this->options['applicable_tax_rate'])) $this->options['applicable_tax_rate'] = 'standard';
	...
	function admin_setup_page() {
		...
		if(isset($_POST['usces_option_update'])) {
			...
			$this->options['applicable_tax_rate'] = ( isset( $_POST['applicable_tax_rate'] ) ) ? $_POST['applicable_tax_rate'] : 'standard';
			if( 'reduced' == $this->options['applicable_tax_rate'] ) {
				$this->options['tax_target'] = 'products';
			}
...
<?php
	...
	function main() {
		...
		if( isset($_REQUEST['order_action']) && $_REQUEST['order_action'] == 'pdfout' ){
			...
			if( !is_user_logged_in() && !$this->is_order( $mid, $oid ) )
				die('No permission');
			if( ( isset( $this->options['tax_display'] ) && 'activate' == $this->options['tax_display'] ) && $this->is_reduced_tax_rate() ) {
	...
	function is_reduced_tax_rate() {

		$reduced = ( isset( $this->options['applicable_tax_rate'] ) && 'reduced' == $this->options['applicable_tax_rate'] ) ? true : false;
		return $reduced;
	}
