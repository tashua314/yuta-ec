...
class WELCARTPAY_SETTLEMENT extends ESCOTT_MAIN
{
	...
	public function initialize_data() {
		...
		if( !array_key_exists( 'welcart', $options['acting_settings'] ) ) {
...
<?php
	...
	public function settlement_update() {
		...
		$options['acting_settings']['welcart']['tenant_id'] = ( isset( $_POST['tenant_id'] ) ) ? $_POST['tenant_id'] : '0001';
						...
						if( 'OK' == $response_data['ResponseCd'] ) {
							...
							//決済
								...
								if( 'ordercompletion' == $res ) {
									...
									$log = array( 'acting'=>$this->paymod_id.'_card', 'key'=>$response_data['MerchantFree1'], 'result'=>'ORDER DATA REGISTERED ERROR', 'data'=>$response_data );
									usces_save_order_acting_error( $log );
									wp_redirect( add_query_arg( array( 'acting'=>$this->paymod_id.'_card', 'acting_return'=>0, 'result'=>0 ), USCES_CART_URL ) );
								}
							}
	...
	public function admin_ajax() {
		...
		case 'gathering_welcartpay_card':
			...
			if( 'OK' == $response_member['ResponseCd'] ) {
				...
				$response_data = apply_filters( 'usces_filter_escott_'.$_POST['mode'].'_history_log', $response_data, $order_id );
				$this->save_acting_history_log( $response_data, $order_id.'_'.$trans_id );
				$res .= $this->settlement_history( $order_id.'_'.$trans_id );
				$data['status'] = $response_data['ResponseCd'];
				$data['acting_status'] = $acting_status;
