...
class SBPS_SETTLEMENT extends SBPS_MAIN
{
	...
	public function initialize_data() {
		...
		if( !array_key_exists( 'sbps', $options['acting_settings'] ) ) {
...
<?php
	...
	public function settlement_update() {
		...
		if( 'on' == $options['acting_settings']['sbps']['card_activate'] || 'on' == $options['acting_settings']['sbps']['conv_activate'] || 'on' == $options['acting_settings']['sbps']['payeasy_activate'] || 'on' == $options['acting_settings']['sbps']['wallet_activate'] || 'on' == $options['acting_settings']['sbps']['mobile_activate'] ) {
...
?>
	...
	<form action="" method="post" name="sbps_form" id="sbps_form">
		...
		<table class="settle_table">
			<tr>
				<th>Yahoo! ウォレット決済</th>
				<td><label><input name="wallet_yahoowallet" type="radio" id="wallet_yahoowallet_sbps_1" value="on"<?php if( isset( $acting_opts['wallet_yahoowallet'] ) && $acting_opts['wallet_activate'] == 'on' ) echo ' checked="checked"'; ?> /><span><?php _e( 'Use', 'usces' ); ?></span></label><br />
					<label><input name="wallet_yahoowallet" type="radio" id="wallet_yahoowallet_sbps_2" value="off"<?php if( isset( $acting_opts['wallet_yahoowallet'] ) && $acting_opts['wallet_activate'] == 'off' ) echo ' checked="checked"'; ?> /><span><?php _e( 'Do not Use', 'usces' ); ?></span></label>
				</td>
			</tr>
			<tr>
				<th>楽天ペイ（オンライン決済）</th>
