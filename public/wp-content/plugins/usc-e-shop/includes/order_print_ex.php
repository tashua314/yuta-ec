...
function usces_pdf_out( $pdf, $data ) {
	...
	if( 0 < $usces_tax->subtotal_standard ) {
		...
		$subtotal_label = sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate'] );
		$subtotal_tax_label = sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate'] );
		...
		$pdf->MultiCell( 22.6, $lineheight, usces_conv_euc( $usces->get_currency( $usces_tax->subtotal_standard ) ), 0, 'R' );
	...
	if( 0 < $usces_tax->subtotal_reduced ) {
		...
		$subtotal_label = sprintf( __( "Applies to %s%%", 'usces' ), $usces->options['tax_rate_reduced'] );
		$subtotal_tax_label = sprintf( __( "%s%% consumption tax", 'usces' ), $usces->options['tax_rate_reduced'] );
		if( 'include' == usces_get_tax_mode() ) {
		...
		$pdf->MultiCell( 37.7, $lineheight, usces_conv_euc( $subtotal_label ), 0, 'C' );
		$pdf->SetXY( 142.9, $line_y );
		$pdf->MultiCell( 22.6, $lineheight, usces_conv_euc( $usces->get_currency( $usces_tax->subtotal_reduced ) ), 0, 'R' );
...
// Detail
function usces_pdfSetDetail( $pdf, $data, $page, $font, $x, $y, $onep, $next_y, $border, $index, $cart, $cart_row, $fontsizes ) {
	global $usces;
